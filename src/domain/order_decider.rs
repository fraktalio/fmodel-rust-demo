...
mod order_decider_tests {
    ...
    use crate::domain::order_decider::{order_decider, Order, OrderDecider};

    #[test]
    fn test() {
        // The Order decider
        let decider: OrderDecider = order_decider();
        // The data
        ...
        // ### EventSourced flavour ### - Test the decider: given EVENTS, when COMMAND, then NEW EVENTS
        let old_events = vec![OrderEvent::Created(OrderCreated {
            identifier: identifier.clone(),
            restaurant_identifier: restaurant_identifier.clone(),
            status: OrderStatus::Created,
            line_items: line_items.clone(),
